...
export async function listarComentariosUsuario(userId: any) {
    try {
...
export async function listarComentariosPorReceta(recetaId: any) {
    try {
        const records = await pb.collection('comentarios').getFullList({
        sort: '-created',
...
// CREAR COMENTARIO
export async function crearComentario({ recetaId, userId, texto, rating }: any) {
    const payload = {
        comentario: texto,
        valoracion: rating,
        receta: recetaId,
...
export async function actualizarComentario(id: any, texto: any, rating: any) {
...
// ELIMINAR COMENTARIO
export async function eliminarComentario(id: any) {
    if (!id) throw new Error('ID de comentario no proporcionado')
    try {
